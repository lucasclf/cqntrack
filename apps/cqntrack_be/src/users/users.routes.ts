...
usersRouter.get("/:username/movies/entries", async (c) => {
  ...
  try {
    const userId = await resolveUserIdByUsername(db, c.req.param("username"));
    const { items, total } = await listMovieEntries(db, userId, parsed.data);
    return c.json(
      PaginatedMovieEntriesResponseSchema.parse({
        items,
        page: parsed.data.page,
        pageSize: parsed.data.pageSize,
        total,
...
usersRouter.get("/:username/movies/favorites", async (c) => {
  const db = createDb(c.env);
  try {
    const userId = await resolveUserIdByUsername(db, c.req.param("username"));
...
usersRouter.get("/:username/movies/lists", async (c) => {
  const db = createDb(c.env);
  try {
    const userId = await resolveUserIdByUsername(db, c.req.param("username"));
    const lists = await listMovieLists(db, userId);
...
usersRouter.get("/:username/movies/lists/:listId", async (c) => {
  const db = createDb(c.env);
  try {
    const userId = await resolveUserIdByUsername(db, c.req.param("username"));
    const detail = await getMovieListDetail(db, userId, c.req.param("listId"));
