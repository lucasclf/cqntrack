...
describe("GET /api/books/favorites", () => {
  it("sem sessão retorna 401", async () => {
    const res = await app.request("/api/books/favorites", undefined, env);
    expect(res.status).toBe(401);
  });
  ...
  it("favoritar via PUT .../entry reflete no GET, mais recente primeiro, sem limite de quantidade", async () => {
    ...
    const getBody = (await getRes.json()) as { items: Array<{ book: { googleBooksId: string } }> };
    expect(getBody.items.map((item) => item.book.googleBooksId)).toEqual(["book-803", "book-801"]);
  });
