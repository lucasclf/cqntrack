...
import styles from "./MyMovieLists.module.css";

type LoadStatus = "loading" | "ready" | "error";

export function MyMovieLists() {
      ...
      <ul className={styles.list}>
        {lists.map((list) => (
          <li key={list.id} className={styles.card}>
            <Link to={`/filmes/listas/${list.id}`} className={styles.cardLink}>
              <span className={styles.name}>{list.name}</span>
              <span className={styles.count}>{list.itemCount} filme(s)</span>
      ...
      {modalOpen && (
        <ListFormModal
          ...
          onSubmit={async (values) => {
            const created = await apiClient.post<MovieList>("/api/movies-lists", values);
