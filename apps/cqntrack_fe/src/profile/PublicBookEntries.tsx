...
function parseStatus(raw: string | null): BookStatus | null {
  return raw !== null && (BOOK_STATUSES as readonly string[]).includes(raw) ? (raw as BookStatus) : null;
}

// Destino das estatísticas clicáveis do perfil público (ver BookStats).
export function PublicBookEntries() {
  ...
  const title = status ? BOOK_STATUS_LABELS[status] : "Livros";
